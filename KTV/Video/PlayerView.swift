...
class PlayerView: UIView {
    ...
    private var statsObservation: NSKeyValueObservation?
    ...
    func set(url: URL) {
        self.player = AVPlayer.init(playerItem: AVPlayerItem(asset: AVURLAsset(url: url)))
    }
    ...
    func seek(to percent: Double) {
        self.player?.seek(to: CMTime(seconds: percent * self.totalPlayTime, preferredTimescale: 1))
    }
    ...
    func forward(to seconds: Double = 10){
        ...
        self.player?.seek(to: CMTime(seconds: currentTIme + seconds, preferredTimescale: 1))
    }
...
extension PlayerView {
    private func setup(player: AVPlayer) {
        self.playObservation = player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 0.5, preferredTimescale: 10), queue: .main) { [weak self, weak player] time in
            guard let self else {
                return
            }
            guard let currentItem = player?.currentItem, currentItem.status == .readyToPlay,
        ...
        self.statsObservation = player.currentItem?.observe(\.status, changeHandler: { [weak self] item, _ in
    ...
    private func unsetPlayer(player: AVPlayer) {
        self.statsObservation?.invalidate()
    ...
    private func setupNotification() {
        NotificationCenter.default.addObserver(self, selector: #selector(self.didPlayToEnd(_:)), name: .AVPlayerItemDidPlayToEndTime, object: nil)
    }
